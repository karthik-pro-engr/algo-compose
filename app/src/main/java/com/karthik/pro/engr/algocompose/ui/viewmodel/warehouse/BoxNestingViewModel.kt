...
import com.karthik.pro.engr.algocompose.domain.stack.NextGreaterElementCalculator.computeNextGreaterElement
    ...
    fun onEvent(event: BoxNestingEvent) {
        when (event) {
            ...
            BoxNestingEvent.ComputeBoxNesting -> {
                viewModelScope.launch {
                    _boxNestingUiState.update {
                        it.copy(boxNestingOrder = computeNextGreaterElement(it.boxSizesList))
                    }
                }
            }
